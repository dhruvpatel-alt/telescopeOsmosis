...
```js
import { __CHAINNAME__ } from '__MODULENAME__';
...
Import the `__CHAINNAME__` object from `__MODULENAME__`. 
...
```js
import { cosmwasm } from "__MODULENAME__";
...
```js
import { ibc } from '__MODULENAME__';
...
```js
import { cosmos } from '__MODULENAME__';
...
Use `getSigning__CHAINNAME__Client` to get your `SigningStargateClient`, with the proto/amino messages full-loaded. No need to manually add amino types, just require and initialize the client:

```js
import { getSigning__CHAINNAME__Client } from '__MODULENAME__';

const stargateClient = await getSigning__CHAINNAME__Client({
  rpcEndpoint,
...
Contract schemas live in `./contracts`, and protos in `./proto`. Look inside of `scripts/codegen.js` and configure the settings for bundling your SDK and contracts into `__MODULENAME__`:
